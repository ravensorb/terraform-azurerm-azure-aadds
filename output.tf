output "resource_group_name" {
  ...
  value       = element(coalescelist(data.azurerm_resource_group.aadds-rg.*.name, azurerm_resource_group.aadds-rg.*.name, [""]), 0)
}

output "resource_group_id" {
  description = "The id of the resource group in which resources are created"
  value       = element(coalescelist(data.azurerm_resource_group.aadds-rg.*.id, azurerm_resource_group.aadds-rg.*.id, [""]), 0)
}

output "resource_group_location" {
  description = "The location of the resource group in which resources are created"
  value       = element(coalescelist(data.azurerm_resource_group.aadds-rg.*.location, azurerm_resource_group.aadds-rg.*.location, [""]), 0)
}
